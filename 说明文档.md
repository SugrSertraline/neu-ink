...
- **私有论文**：用户创建，仅自己可见可编辑
- **笔记**：必须关联到论文Block，完全私有，删除论文时级联删除
...
#### 4.1 笔记管理
- **创建笔记**：为论文的某个Block创建笔记，输入笔记内容和可选标签，需验证论文是否在用户的个人库中且blockId有效
- **编辑笔记**：修改笔记内容或标签
- **删除笔记**：删除指定笔记
- **查看某论文的笔记**：获取当前用户在某篇论文下的所有笔记列表
- **查看所有笔记**：获取当前用户的所有笔记（跨论文），支持分页，包含笔记所属论文的基本信息
...
### 4.1 管理员判断
通过用户的username是否为'admin'来判断是否为管理员
...
### 5.1 级联删除
- **删除论文时**：同时删除所有用户与该论文的关联记录（UserPaper）以及所有相关笔记（Note）
- **删除用户时**：删除用户创建的所有私有论文（及其级联数据）、删除该用户的所有关联记录和所有笔记
