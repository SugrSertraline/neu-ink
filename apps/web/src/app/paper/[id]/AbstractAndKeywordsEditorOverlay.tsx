...
import AbstractAndKeywordsEditor from '@/components/paper/editor/AbstractAndKeywordsEditor';
import { useCallback, useEffect, useMemo, useState } from 'react';

interface AbstractAndKeywordsEditorOverlayProps {
...
export default function AbstractAndKeywordsEditorOverlay({
  abstract,
  keywords,
  onSubmit,
  onCancel,
  ...
  autoSave = false,
  autoSaveDelay = 2000,
}: AbstractAndKeywordsEditorOverlayProps) {
    ...
    >
      ...
      >
        <header className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">编辑摘要和关键词</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">点击下方保存按钮将保存您的摘要和关键词修改到服务器。</p>
          </div>
