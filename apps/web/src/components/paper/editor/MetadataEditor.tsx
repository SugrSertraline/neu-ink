...
const toFormState = (metadata: PaperMetadata): MetadataFormState => ({
...
const toMetadata = (form: MetadataFormState): PaperMetadata => {
...
const validateForm = (
...
): { ok: boolean; issues: string[]; payload?: PaperMetadata } => {
  const issues: string[] = [];
  ...
  return { ok: true, issues: [], payload: toMetadata(form) };
};
...
export default function MetadataEditor({
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async event => {
    ...
    setSubmittingError(null);
    try {
      await Promise.resolve(onSubmit(verdict.payload));
    } catch (err) {
