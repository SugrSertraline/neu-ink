  ...
  const handleContextMenu = useCallback(
    (event: React.MouseEvent<HTMLElement>) => {
      ...
      const isValidTarget = () => {
        ...
        // 检查是否在章节标题区域
        const sectionTitle = target.closest('.flex.items-baseline.gap-3');
...
export function BlockContextMenu({
  ...
  const { canEditContent } = usePaperEditPermissionsContext();
  const { switchToEdit } = useEditorStore();
        ...
        if ('content' in translatedBlock && translatedBlock.content) {
          ...
          requestAnimationFrame(async () => {
            try {
              ...
              console.error('翻译切换编辑状态时出错:', error);
              toast.error('翻译失败', { id: 'translate-block' });
            }
          });
        } else {
